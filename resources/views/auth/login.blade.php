...
<head>
    ...
    <!-- Main Theme Js -->
    <script src="{{asset('admin/js/authentication-main.js')}}"></script>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            ...
                            $('#password').val(''); // Clear password on general failure
                            // No auto-clear for email on general failure as it might be correct
                        }
                    },
                    error: function(xhr) {
                        $btn.prop('disabled', false).html(originalBtnHtml);
                        if (xhr.status === 422) { // Validation errors
                            ...
                            if (errors.email) {
                                ...
                                setTimeout(() => $('#email-error').text(''), 2000);
                            }
                            if (errors.password) {
                                $('#password-error').text(errors.password[0]);
                                setTimeout(() => $('#password-error').text(''), 2000);
                            }
                            $('#email').val(''); // Clear email on validation error
                            $('#signin-password').val(''); // Clear password on validation error
                        } else if (xhr.status === 401) { // Invalid credentials
                            toastr.error(xhr.responseJSON.message || 'Invalid credentials.');
                            $('#signin-password').val(''); // Clear password on invalid credentials
                            setTimeout(() => $('#signin-password').val(''), 2000);
                        } else {
